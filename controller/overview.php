...
class overview
{
	...
	public function base()
	{
		...
		// get pending (inactive) articles, for moderators only
		...
		if ($this->auth->acl_get('m_wiki_view_inactive'))
		{
			...
			$sql = 'SELECT article_id, article_title, article_url, article_last_edit
					FROM ' . $this->article_table . '
					WHERE article_approved=0
					ORDER BY article_url ASC, article_last_edit DESC';
			$result = $this->db->sql_query($sql);

			$seen_urls = array();
			while ($pending_wiki_article = $this->db->sql_fetchrow($result))
			{
				// Several pending versions can share a URL; keep only the most recent one
				if (isset($seen_urls[$pending_wiki_article['article_url']]))
				{
					continue;
				}
				$seen_urls[$pending_wiki_article['article_url']] = true;

				// If an active version already exists and is at least as recent as this
				// pending one, approving it would regress the live article - only allow
				// a direct one-click approve when this pending edit is genuinely the newest
				$sql2 = 'SELECT article_last_edit
						FROM ' . $this->article_table . "
						WHERE article_url = '" . $this->db->sql_escape($pending_wiki_article['article_url']) . "'
							AND article_approved = 1";
				$result2 = $this->db->sql_query_limit($sql2, 1);
				$active_last_edit = $this->db->sql_fetchfield('article_last_edit');
		...
		$this->template->assign_vars(array(
			'S_AUTH_EDIT'	=> $this->auth->acl_get('u_wiki_edit'),
		));
